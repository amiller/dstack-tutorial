#!/usr/bin/env python3
"""
Add a compose hash to an AppAuth contract's whitelist.

Usage:
    python3 add_compose_hash.py <app_auth_address> <compose_hash>
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 add_compose_hash.py <app_auth_address> <compose_hash>")
        print("Example: python3 add_compose_hash.py 0x5a367973... 0xabcd1234...")
        sys.exit(1)
    ...
    # Ensure compose_hash is bytes32 (64 hex chars after 0x)
    compose_hash_bytes = bytes.fromhex(compose_hash.replace("0x", "").zfill(64))

    contract = w3.eth.contract(address=app_auth_address, abi=APP_AUTH_ABI)

    # Check if already allowed
    is_allowed = contract.functions.allowedComposeHashes(compose_hash_bytes).call()
    if is_allowed:
        print(f"Compose hash {compose_hash} is already allowed")
        sys.exit(0)

    print(f"Adding compose hash to AppAuth contract...")
    print(f"  AppAuth:     {app_auth_address}")
    print(f"  ComposeHash: {compose_hash}")
    print(f"  Owner:       {account.address}")

    tx = contract.functions.addComposeHash(compose_hash_bytes).build_transaction({
    ...
    if receipt.status == 1:
        print(f"  ✅ Compose hash added successfully")
    else:
