#!/usr/bin/env python3
"""
Verify TEE oracle signature chain locally.

Prerequisites:
  pip install eth-account eth-keys requests
...
def verify_signature_chain(data, expected_kms_root):
    ...
    1. KMS root signed app key
    2. App key signed derived key
    ...
    if message_signer.lower() != expected_signer.lower():
        print("  FAILED: Message signature mismatch")
        return False

    print("  OK: Message signature verified")
    return True

def main():
    print("TEE Oracle Signature Chain Verification")
    print("=" * 60)
    ...
    if verify_signature_chain(data, KMS_ROOT_ADDRESS):
        ...
        print("\nThis signature chain can be verified on-chain.")
        return True
    else:
        print("\nVerification failed!")
        return False
